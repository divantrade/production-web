...
export default function VideoPlayer({
  ...
  const [isPlaying, setIsPlaying] = useState(false); // Always start as false to avoid hydration mismatch
  ...
  useEffect(() => {
    ...
    // Generate embed URL on client side only
  ...
  const getEmbedUrl = () => {
    if (videoSource.type === 'youtube' && videoSource.id) {
      const params = new URLSearchParams({
        ...
        playsinline: '1',
        enablejsapi: '1',
        origin: isClient && typeof window !== 'undefined' ? window.location.origin : 'http://localhost:3000',
      });

      // Use youtube-nocookie.com for better privacy and less blocking
      return `https://www.youtube-nocookie.com/embed/${videoSource.id}?${params.toString()}`;
    }
  ...
  const renderVideo = () => {
    ...
    // YouTube or Vimeo iframe - only render on client with valid URL
    if (!isClient || !embedUrl) {
      return (
        <div className="absolute inset-0 bg-gray-900 flex items-center justify-center">
          <div className="text-white">Loading video...</div>
        </div>
  ...
  return (
    ...
    >
      ...
      {isClient && (videoSource.type === 'youtube' || videoSource.type === 'vimeo') && showPlayButton && !isPlaying && (
